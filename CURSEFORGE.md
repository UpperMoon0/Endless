...
Endless lets you build higher and dig deeper than ever before. In v0.6, you can configure your world's build range anywhere from Y=-8,000,000 through Y=7,999,999 without turning every chunk into a millions-of-block-tall section array.
...
* **Compatible with Existing Worlds** — v0.6 preserves the fail-closed migration safeguards from v0.4 so old extended sections are not silently discarded. Back up important worlds before upgrading.
...
* **Server-Authoritative Multiplayer** — Servers synchronize the configured logical range and dense layout before sparse world data is used. Extended worlds require Endless v0.6-compatible clients.
...
To unlock the complete v0.6 representation envelope:
...
Because v0.6 uses sparse storage, simply widening the configured range no longer allocates a dense section array for every possible Y-level. Memory and storage grow primarily with the extended pages that actually contain data.

## Existing Worlds

Fresh v0.6 worlds always keep a vanilla-sized `[-64,320)` dense core internally, even when the configured logical range is millions of blocks tall.
...
* **Representation envelope** — v0.6 is practically unbounded, not mathematically infinite. The supported logical range is `[-8000000, 8000000)`.
...
* **Multiplayer clients** — Sparse v0.6 worlds require Endless v0.6-compatible clients; vanilla clients cannot join an extended-range Endless server.
